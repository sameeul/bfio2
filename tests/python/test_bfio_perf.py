...
def bfio_loop_all_tiles(file_name, num_threads=1):
    ...
    print(
        f"Test Name : {test_name} Init time : {init_time - start_time} IO time : {end_time-init_time} "
    )
...
def bfio2_loop_all_tiles_planned(file_name, num_threads=1):
    ...
    br = NewBioReader(file_name, num_threads)
    init_time = time.time()
    for ind_, tile in br(tile_size=[1024, 1024], tile_stride=[1024, 1024]):
        tile.sum()
            ...
            for thread in thread_cfg:
                for test_type in test_dict:
                    ...
                    for test_name in test_data:
                        ...
                        init_result = (
                            init_result + ", " + test_name + ", " + str(result[0])
                        )
