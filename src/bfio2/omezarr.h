...
public:
    ...
    /// @brief OmeZarrTileLoader constructor
    /// @param numberThreads Number of threads associated
    /// @param filePath Path of zarr file
    OmeZarrTileLoader(
        size_t numberThreads,
        std::string const& filePath)
        : BfioTileLoader<DataType>("OmeZarrTileLoader", numberThreads, filePath)
    {
        // Open the file
    ...
    void loadTile(std::shared_ptr<std::vector<DataType>> &dest, size_t pixel_row_index, size_t pixel_col_index, size_t pixel_layer_index){
        ...
        size_t c_index = tmp_cz / num_tile_depth;
        size_t z_index = tmp_cz % num_tile_depth;
    ...
    [[nodiscard]] size_t numberChannels() const override { return num_ch_;}

    size_t numTsteps() const override { return num_tsteps_; }
