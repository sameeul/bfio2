...
using tile_data_type = uint32_t;

class OmeTiffLoader{

    private:
        std::shared_ptr<BfioTileLoader<tile_data_type>> tile_loader_;
        ...
        std::shared_ptr<fl::FastLoaderGraph<fl::DefaultView<tile_data_type>>> fast_loader_;
        ...
        void SetZCT();
        void CopyToVirtualTile(const Seq& rows, const Seq& cols, size_t i, size_t j, std::shared_ptr<std::vector<tile_data_type>> virtual_tile_data, size_t offset);

    public:
        OmeTiffLoader(const std::string &fNameWithPath, const int num_threads=1);
        ~OmeTiffLoader();
 
        std::shared_ptr<std::vector<tile_data_type>> GetTileData(size_t const row, size_t const col, size_t const layer=0, size_t const channel=0, size_t const tstep=0);
        std::shared_ptr<std::vector<tile_data_type>> GetTileDataByIndex(size_t const tile_index, size_t const channel=0, size_t const tstep=0);
        std::shared_ptr<std::vector<tile_data_type>> GetVirtualTileData(const Seq& rows, const Seq& cols, const Seq& layers = Seq(0,0), const Seq& channels = Seq(0,0), const Seq& tsteps = Seq(0,0));
        std::shared_ptr<std::vector<tile_data_type>> GetVirtualTileDataStrided(const Seq& rows, const Seq& cols, const Seq& layers = Seq(0,0), const Seq& channels = Seq(0,0), const Seq& tsteps = Seq(0,0));
        ...
        std::shared_ptr<std::vector<tile_data_type>> GetViewRequests(size_t const index_row_pixel_min, size_t const index_row_pixel_max,
