...
class BioReader:
    ...
    def data(self, row=None, col=None, layer=0, channel=0, tstep=0):
        if col is not None:
            return self._image_reader.get_tile_data_2d_by_row_col_layer_channel(
                row, col, layer, channel, tstep
            )
        else:
            return self._image_reader.get_tile_data_2d_by_index_channel(
                row, channel, tstep
            )
    ...
    def row_tile_count(self):
        if self._row_tile_count is None:
    ...
    def column_tile_count(self):
        if self._column_tile_count is None:
    ...
    def physical_size_x(self):
        if self._physical_size_x is None:
            self._physical_size_x = self._image_reader.get_metadata_value(
                "PhysicalSizeX"
            )

        if self._physical_size_x_unit is None:
            self._physical_size_x_unit = self._image_reader.get_metadata_value(
                "PhysicalSizeXUnit"
            )
    ...
    def physical_size_y(self):
        if self._physical_size_y is None:
            self._physical_size_y = self._image_reader.get_metadata_value(
                "PhysicalSizeY"
            )

        if self._physical_size_y_unit is None:
            self._physical_size_y_unit = self._image_reader.get_metadata_value(
                "PhysicalSizeYUnit"
            )
    ...
    def physical_size_z(self):
        if self._physical_size_z is None:
            self._physical_size_z = self._image_reader.get_metadata_value(
                "PhysicalSizeZ"
            )

        if self._physical_size_z_unit is None:
            self._physical_size_z_unit = self._image_reader.get_metadata_value(
                "PhysicalSizeZUnit"
            )
    ...
    def samples_per_pixel(self):
        if self._samples_per_pixel is None:
    ...
    def __getitem__(self, keys):
        ...
        if row_step != 1 or col_step != 1:
            return self._image_reader.get_virtual_tile_data_5d_strided(
                rows, cols, layers, channels, times
            )
        else:
            return self._image_reader.get_virtual_tile_data_5d(
                rows, cols, layers, channels, times
            )
    ...
    def _parse_slice(self, keys):
        ...
        # may be also setup a failsafe / warning for too large image
        if not isinstance(keys, tuple):
            if (
                ...
                raise ValueError(
                    "If only a single index is supplied, it must be an empty slice ([:])."
                )
        ...
        # If not an empty slice, parse the key tuple
            ...
            if len(keys) > 5:
                raise ValueError(
                    "Found {} indices, but at most 3 indices may be supplied.".format(
                        len(keys)
                    )
                )
            ...
            if keys[0] == Ellipsis:
                ...
                if keys[-1] == Ellipsis:
                    raise ValueError(
                        "Ellipsis (...) may be used in either the first or last index, not both."
                    )
            ...
            for dim, key in zip(dims, keys):
                if isinstance(key, slice):
                    ...
                    stop = self._DIMS.get(dim, 0) if key.stop is None else key.stop
    ...
    def __iter__(self):
        ...
        # request views
        self._image_reader.send_iterator_view_requests(
        ...
        for data in self._image_reader.__iter__():
            ...
            yield (
                row_index,
                col_index,
            ), self._image_reader.get_iterator_requested_tile_data(
