...
void test1()
{
    ...
    std::shared_ptr<std::vector<uint32_t>> tileData = imgLoader.GetTileDataByRowColLayer(0,0,0);
    size_t sum = 0;
    sum += partial_sum(tileData, 500, 1023,700,1023, 1024);
    tileData = imgLoader.GetTileDataByRowColLayer(0,1,0);
    sum += partial_sum(tileData, 500, 1023, 0,1070-1024, 56);
    tileData = imgLoader.GetTileDataByRowColLayer(1,0,0);
    sum += partial_sum(tileData, 0, 1050-1024,700,1023, 1024);
    tileData = imgLoader.GetTileDataByRowColLayer(1,1,0);
...
void test9()
{
    ...
    for (int i=0; i<2; ++i){
        for (int j=0; j<2; ++j){
            std::shared_ptr<std::vector<uint32_t>> tileData = imgLoader.GetTileDataByRowColLayer(i,j,0);
            size_t sum = std::accumulate(tileData->begin(), tileData->end(), size_t(0));        
            std::cout << "Tile id " << i << ", sum = "<< sum <<std::endl;
        }
